...
cbind(rnorm(history_days,sd=0.01),rep(0,history_days))[,sample(c(1,rep(2,1)),length(stocks),replace=TRUE)]+ # 50% with a low vol common factor
cbind(rnorm(history_days,sd=0.02),rep(0,history_days))[,sample(c(1,rep(2,10)),length(stocks),replace=TRUE)]+ # 5% with a medium vol common factor
cbind(rnorm(history_days,sd=0.04),rep(0,history_days))[,sample(c(1,rep(2,100)),length(stocks),replace=TRUE)] # 0.5% with a high vol common factor
)*matrix(rep(rep(c(0,1),times=c(2,5)),length.out=1000),ncol=1)[,rep(1,length(stocks))] # weekends 

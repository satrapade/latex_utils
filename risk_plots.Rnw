...
liquidity_vs_marginal_risk<-make_pair_layout(
    layout_matrix=df2matrix(data.table(
...
pair_stat_matrix<-mapply(function(m){
  ntable(data.table(m),add_rownames=FALSE,add_header=FALSE,alternating=FALSE)
},mapply(
  make_matrix_from_args,
  gross_txt=rep("{\\small gross}",nrow(duke_pair_exposure)),
  gross=apply(abs(duke_pair_exposure),2,sum),
  marginal_risk_txt=rep("{\\small mrisk}",nrow(duke_pair_exposure)),
  marginal_risk=round(sd(duke_local_pnl)-apply(duke_drop_one_pair_pnl,2,sd),digits=0),
  unit_marginal_risk_txt=rep("{\\small umrisk}",nrow(duke_pair_exposure)),
  unit_marginal=round((sd(duke_local_pnl)-apply(duke_drop_one_pair_pnl,2,sd))/apply(abs(duke_pair_exposure),2,sum),digits=0),
  liquidity_txt=rep("{\\small adv}",nrow(duke_pair_exposure)),
  liquidity=round(apply(abs(duke_pair_days),2,max),digits=0),
  MoreArgs = list(template_matrix=rbind(c(1,2,3,4),c(5,6,7,8))),
  SIMPLIFY=FALSE
))

detailed_report<-data.table(
  pair=icon(icon_name=colnames(duke_pair_exposure),width="20mm",height="20mm"),
  details=pair_stat_matrix
)

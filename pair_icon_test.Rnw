...
pair_price_action<-function(pair,pair_local_pnl=duke_pair_local_pnl,w=21){
  pnl<-tail(pair_local_pnl[,pair],21)
  txt<-make_tikz_price_action(pnl,width=30,height=30,units="mm")
  txt
}
...
pair_technicals<-function(
  ...
  short_px<-cumprod(c(0,tail(short_pnl,w))+1)
  rel_px<-rescale(long_px/short_px)
  rng<-100*(tail(re_px,1)-min(rel_px))/diff(range(rel_px))
  rsi<-round(tail(rsi(diff(rel_px),14),1),digits=0)
...
pair_longs<-mapply(function(i)pair_leg(pair=i,direction=1),pairs)
pair_shorts<-mapply(function(i)pair_leg(pair=i,direction=(-1)),pairs)
pair_12m_sparks<-mapply(pair_sparkline,pairs,MoreArgs = list(w=360))
pair_6m_sparks<-mapply(pair_sparkline,pairs,MoreArgs = list(w=182))
pair_1m_sparks<-mapply(pair_sparkline,pairs,MoreArgs = list(w=30))
pair_latest_action<-mapply(pair_price_action,pairs,MoreArgs = list(w=21))
pair_pm<-mapply(pair_with_pm_color,pairs)
...
icon<-function(pair){
  paste0(
    paste0("\\multicolumn{1}{@{}c@{}}{",pair_pm,"}"),
    paste0("\\multicolumn{1}{@{}c@{}}{",pair_latest_action,"}"),
    paste0("\\multicolumn{1}{@{}c@{}}{",pair_stats,"}"),
    pair_longs,
    pair_shorts
  )
}
